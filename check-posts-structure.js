...
async function checkPostsStructure() {
  try {
    ...
    const { data: legalPropertyPosts, error: legalPropertyError } = await supabase
      .from('posts')
      .select('*')
    ...
    if (legalPropertyError) {
      ...
      console.log(`Found ${legalPropertyPosts.length} legal/property-related posts:`);
      legalPropertyPosts.forEach((post, index) => {
