      ...
      const loadPosts = async () => {
        try {
          const postsResponse = await fetch(
            `/api/posts/published?language=${language}`
          );
          if (postsResponse.ok) {
      ...
      {posts.length === 0 ? (
        <div className="text-center py-20 bg-white dark:bg-slate-800 rounded-3xl shadow-sm border border-gray-100 dark:border-slate-700">
          <p className="text-gray-500 dark:text-gray-400 text-lg">
            {language === "zh" ? "暂无相关文章" : "No posts found"}
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8">
          {posts.map((post) => (
            <Link
              ...
              className="block"
            >
              <Card className="rounded-3xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 hover:scale-105 bg-white dark:bg-slate-800 border-none cursor-pointer h-full">
                ...
                <Box className="p-4 space-y-3 flex-1 flex flex-col">
                  <h3 className="text-xl font-bold line-clamp-2 text-gray-900 dark:text-gray-100">{post.title}</h3>
                  <Text size="2" className="text-gray-500 dark:text-gray-400">
                    ...
                    <span className="text-gray-400 dark:text-gray-500"> • 5 min read</span>
                  </Text>
                  <Text size="2" className="text-gray-600 dark:text-gray-300 line-clamp-3 flex-1">
                  ...
                  <div className="pt-4 flex items-center justify-between border-t border-gray-100 dark:border-slate-700 mt-auto">
                    <div className="inline-flex items-center gap-2 whitespace-nowrap text-gray-700 dark:text-gray-300">
                      ...
                      />
                      <span className="text-sm font-medium">{post.author || "Marie Nian"}</span>
                    </div>
                        ...
                        <>
                          ...
                          {toggles.aiSummaries && (
                            <div className="flex items-center gap-1">
                              ...
                              <span>{postStats[post.id].aiSummaries || 0}</span>
                            </div>
                          )}
                        </>
                      ) : (
